...
class FeatureRichStackingFraudMLP(nn.Module):
    ...
    def __init__(
        ...
        use_pairwise_products: bool = False,
        use_pairwise_absdiff: bool = False,
        ...
        self.use_pairwise_products = use_pairwise_products
        self.use_pairwise_absdiff = use_pairwise_absdiff
        ...
        feature_dim = self._compute_feature_dim(input_dim)
        ...
        self.config = {
            ...
            "use_pairwise_products": use_pairwise_products,
            "use_pairwise_absdiff": use_pairwise_absdiff,
            "use_summary_stats": use_summary_stats,
            "use_quadratic": use_quadratic,
            "eps": eps,
        }

    def _num_pairs(self, m: int) -> int:
        return m * (m - 1) // 2

    def _compute_feature_dim(self, input_dim: int) -> int:
        dim = input_dim

        if self.use_logits:
            dim += input_dim

        if self.use_pairwise_products:
            dim += self._num_pairs(input_dim)

        if self.use_pairwise_absdiff:
            dim += self._num_pairs(input_dim)
        ...
        if self.use_quadratic:
            dim *= 2  # append square of all currently-built features

        return dim

    def _pairwise_products(self, x: torch.Tensor) -> torch.Tensor:
        parts = []
        _, m = x.shape
        for i in range(m):
    ...
    def _pairwise_absdiff(self, x: torch.Tensor) -> torch.Tensor:
        parts = []
        _, m = x.shape
        for i in range(m):
            for j in range(i + 1, m):
                parts.append(torch.abs(x[:, i] - x[:, j]).unsqueeze(1))
        if not parts:
            return x.new_zeros((x.shape[0], 0))
        return torch.cat(parts, dim=1)
    ...
    def _summary_stats(self, x: torch.Tensor) -> torch.Tensor:
        ...
        rangev = maxv - minv
        return torch.cat([mean, std, minv, maxv, rangev], dim=1)

    def build_features(self, meta_x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            meta_x: [B, M] probabilities from member models

        Returns:
            engineered meta-features [B, K]
        """
        features = [meta_x]

        if self.use_logits:
            p = meta_x.clamp(self.eps, 1.0 - self.eps)
            pseudo_logits = torch.log(p / (1.0 - p))
            features.append(pseudo_logits)

        if self.use_pairwise_products:
            features.append(self._pairwise_products(meta_x))

        if self.use_pairwise_absdiff:
            features.append(self._pairwise_absdiff(meta_x))

        if self.use_summary_stats:
            features.append(self._summary_stats(meta_x))

        out = torch.cat(features, dim=1)

        if self.use_quadratic:
            out = torch.cat([out, out.square()], dim=1)
...
class StackedFraudEnsemble(nn.Module):
    ...
    def forward_logits(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if hasattr(self.stacker, "build_features"):
            meta_x = self.stacker.build_features(meta_x)
...
class XGBoostAEWrapper(ProbModelWrapper):
    def __init__(self, xgb_model, autoencoder):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feats_np = features.detach().cpu().numpy()
        probs_np = self.xgb_model.predict_proba(feats_np)[:, 1]

        probs = torch.from_numpy(probs_np).to(device=device, dtype=torch.float32)
        return probs